...
% we generally conceptualise frequencies as the number of cycles per unit
% time but the can also be conceptualised as the 'speed' of an oscillation
% from one timepint to the other (generally called instantaneous frequency)
...
% here use Hilbert just to transform a real singnal in a complex one 
angels = angle(hilbert(signal));
% compute the derivative of the unwrapped angles distribution to have a
% measure of how fast each frequency changes at each point, that is, the
% instantaneous frequency; scale by 2pi/srate to go from radiants to
% frequencies
...
title('Frequency domain via FFT')
...
legend({'Estimated';'Ground truth';'mean-smoothing'})
...
%% narrowband filter around 10 Hz (FIR), i.e. alpha band
...
figure(4), clf
plot(time,eegdata, time,feegdata ,'linew',1.5)
xlabel('Time (s)'), ylabel('Amplitude')
zoom on
...
set(gca,'ylim',[5 15])
title('Median filter of the instantaneous frequencies')
zoom on
