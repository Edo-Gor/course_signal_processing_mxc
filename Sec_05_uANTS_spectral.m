...
phases_g = char('pi/7',  'pi/8',  'pi',  'pi/2',  '-pi/4');

% define time...
% note for the future: it's veriy important to have odd numbers of
% timepoints to create morlet wavelet
time = -1:1/srate:1;
...
for sinei=1:length(amplit)
    ...
    title(sprintf('Sinewave of frequency %d, amplitude %d, and phase %s',frex(sinei),amplit(sinei),phases_g(sinei,:))) % print looping title (use sprintf())
end
...
mag = sqrt(compnum*conj(compnum)); % same as below
mag = abs(compnum);
phs = angle(compnum);
...
srate = 1000; % in Hz
time  = -1:1/srate:1; % from -1 to 1 in jumps of 1/srate (i.e. freq in time)
...
csw = ampl.*exp(1i*2*pi.*time.*freq);
...
% plot in 3D
subplot(212)
plot3(time,real(csw),imag(csw),'linew',2)
...
for fi=1:length(dps)
    ...
    sinew = exp(1i*2*pi*time*sinefrex(fi));
...
%             Surprise surprise, if it's a complex sinewave than the
%             phase-dependecy disappear!

%%

%% A movie showing why complex sine waves are phase-invariant
% basically you will notice that in the complex plane the complex dot
% product stays constantly away from the origin (magnitude preserved),
% while in the real plane the real-valued dot product will oscillate back
% and forth trough the value zero
...
% remember, the Fourier coefficient is nothing more that the dot product
% between the signal (real-valued) and a sinewave (complex-valued) at a
% given frequency; the result is a complex number in that can be described
% in the shape of me^(ik), where m is the amplitude (m^2 is the power), and
% k is the phase angle
...
% yet another possibility (the complex number times its conjugate); this is
% also the fastest computational option to get POWER (use sqrt() if
% amplitude needed)
% amp1 = fCoefs1.*conj(fCoefs1);
% amp2 = fCoefs2.*conj(fCoefs2);
% amp3 = fCoefs3.*conj(fCoefs3);

figure(9), clf

% plot amplitude spectrum
subplot(211)
plot(hz,2*amp1(1:length(hz)),'ro-','linew',2), hold on
plot(hz,2*amp2(1:length(hz)),'bp-','linew',2)
plot(hz,2*amp3(1:length(hz)),'ks-','linew',2)
...
phs1 = atan2(imag(fCoefs1),real(fCoefs1)); % 4-quadrant inverse tangent of ratio imag()/real()
...
% plot phase spectrum
subplot(212)
plot(hz,phs1(1:length(hz)),'ro-','linew',2), hold on
plot(hz,phs2(1:length(hz)),'bp-','linew',2)
plot(hz,phs3(1:length(hz)),'ks-','linew',2)
set(gca,'xlim',[freq-3 freq+3])
xlabel('Frequency (Hz)'), ylabel('Phase (rad.)')
title('Phase spectrum (all freqs exept 6 are basically random angles because amplitude approx. zero)')

% notice that here the phase angles for all the frequencies that are not
% 6Hz are basically uninterpretable; that's the case because for all freqs
% except 6, the amplitude is ~zero, and therefore the angle is undefined
...
% They start slowing down (aliasing)

%%% QUESTION: What happens to the sine waves as they approach f=.5?
% They get faster and faster as they approach the higher possible frequency
% to be reconstructed (i.e., 1/2 the sampling rate)
...
% extract amplitudes
% ampls = abs(fCoefs);
ampls = 2*abs(fCoefs)/pnts; % the DC is also multiplied, here irrilevant but be careful
...
% No, the original code is not normalised
...
% Sure, it depends on length of signal and on negative frequecies
...
% normalise by negative freqs without including the DC in the *2 scaling
signalX1(2:length(hz)) = 2*abs(signalX1(2:length(hz)));
signalX2(2:length(hz)) = 2*abs(signalX2(2:length(hz)));
signalX3(2:length(hz)) = 2*abs(signalX3(2:length(hz)));

% plot signals in the frequency domain
subplot(212)
plot(hz,signalX1(1:length(hz)),'bo-','linew',2,'markersize',8), hold on
plot(hz,signalX2(1:length(hz)),'rd-','linew',2,'markersize',8)
plot(hz,signalX3(1:length(hz)),'k*-','linew',2,'markersize',8)
...
%             (Yes, of course you can! Do it!). Done !
...
ampl = 2*abs(fft(eegdata)/npnts); % DC not an issue as data high-pass filtered
powr = ampl.^2;

figure(14), clf, hold on
plot(hz,ampl(1:length(hz)),'k','linew',1)
plot(hz,powr(1:length(hz)),'r','linew',1)
...
% (1) a 1/f shape, a component around 8-10 Hz, a huge artifact limited at
% 50 Hz (line noise)

%%% QUESTION: What do you notice about the difference between the amplitude
%             and power spectra?
% the power spectrum maybe highlight the features better than the amplitude
% (notice that most of the spectrum is below value of 1)

%%% QUESTION: Can you see the ~10 Hz oscillation in the raw time series data?
% somewhere, if you zoom in a lot (~1 sec window)
...
% compute the power spectrum of each 2-second epoch (should have same size
% as .data cube [64 x 2048 x 63]); you could use a double for loop over
% channels and trials, but notice that the fft() function can take as 3rd
% input the dimention over which to compute the fft() in matrices with
% higher dimentions, therefore implicitly implementing the loops

chanpowr = (2*abs( fft(EEG.data,[],2)/EEG.pnts )).^2; % dim 2 is the time dimension, the one we need


% then average over trials (3rd dimention of the data cube)
...
% convert to indices
freqidx = dsearchn(hz',alphabounds'); % this damn function always takes COLUMN vectors as inputs
...
for fi=1:N
    ...
    fourierSine = exp( 1i*2*pi*(fi-1)*fourierTime ) *fourierCoefs(fi); 
...
set(gca,'xlim',[0 2001],'ylim',[-12 12])
...
% compute the FFT of the same signal with different FFT length; notice that
% the padding parameters takes the number of signal point + those we want
% to add to zero-pad (not just the zero-padding points)
...
subplot(212)
stem(hz,2*abs(signalX(1:length(hz))),'s-','markerface','k')
xlabel('Frequency (Hz)'), ylabel('amplitude')
set(gca,'xlim',[0 20])
title('Frequency domain')
...
ampl1 = abs(interp1(linspace(t(1),t(end),10),10*rand(1,10),t,'spline')); % add noise
ampl2 = mean(ampl1);
...
subplot(212)
stem(hz,2*abs(signal2X(1:length(hz))),'ro-','markerface','r','linew',2)
hold on
stem(hz,2*abs(signal1X),'bs-','markerface','k','linew',2)
...
subplot(211)
plot(signal)
title('Phase-slip data (time domain)')
...
xlabel('Frequency (Hz)')
ylabel('Power (\muV^2)')
title('Frequency domain ERP (Welch''s method)')
