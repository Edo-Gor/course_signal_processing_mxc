...
set(gca,'xlim',[-1.1 1.1],'ylim',[-1.1 1.1]*ampl); 
xlabel('Time (ms)')
...
% now plot the result
figure(2), clf
plot(time,sum(sine_waves))  % sum the timeseries and ploz
title('\textbf{A sum of sine waves}','Interpreter','latex')
subtitle('Frequencies: $[5, 15, 10, 5, 7,]$, and Amplitudes : $[\pi/7, \pi/8, \pi, \pi/2, -\pi/4]$','Interpreter','latex')
xlabel('Time (s)','Interpreter','latex'), ylabel('Amplitude (arb. units)','Interpreter','latex')
...
for fi=1:length(frex)
    ...
    title(sprintf('Sinewave of frequency %d',frex(fi))) % print looping title (use sprintf())
    axis([ time([1 end]) -max(amplit)-2 max(amplit)+2 ])
end
...
% empirical FWHM
gwinN   = gwin./max(gwin);  % normalise the gaussian 
