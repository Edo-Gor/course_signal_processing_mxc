...
% FFT over all channels, compute power and then average over trials
powers = (2*abs(fft(EEG.data,[],2)/pnts)).^2;
powers = mean(powers,3);
...
freqwin1 = [ 4 11 ];
tidxW1 = dsearchn(hz',freqwin1');

freqwin2 = [ 11 20 ]; 
tidxW2 = dsearchn(hz',freqwin2');

% power in first and second spectral windows
powLo = mean(powers(:,tidxW1(1):tidxW1(2)),2);
powHi = mean(powers(:,tidxW2(1):tidxW2(2)),2);
...
subplot(223)
topoplotIndie(powLo, EEG.chanlocs,'numcontour',0,'electrodes','numbers','shading','interp');
title(['Data: ' num2str(round(hz(tidxW1(1)))) ' - ' num2str(round(hz(tidxW1(2)))) ' Hz'])

subplot(224)
topoplotIndie(powHi, EEG.chanlocs,'numcontour',0,'electrodes','numbers','shading','interp');
title(['Data: ' num2str(round(hz(tidxW2(1)))) ' - ' num2str(round(hz(tidxW2(2)))) ' Hz'])


%% quantify fit via R2

% data to each dipole projection; notice that corrcoeff() takes colums as input
