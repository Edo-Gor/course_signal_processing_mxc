...
timev = timev - mean(timev); % yet another way to zero-center
...
subplot(311)
plot(timev,fm,'k','linew',1.5)
xlabel('Time (s)'), ylabel('Frequency (Hz)')

% plot the chirp
subplot(312)
plot(timev,churp,'k','linew',1.5)
xlabel('Time (s)'), ylabel('Amplitude (a.u.)')

% power spectrum
subplot(313)
plot(linspace(0,srate,pnts),abs(fft(churp)).^2,'k','linew',1.5)
xlabel('Time (s)'), ylabel('Amplitude (a.u.)')
set(gca,'xlim',[0 maxfreq*2])
title('Static FFT (notice the impact of the frequency non-stationarity)')
...
% FFT of data (doesn't change over frequencies!) but you have to recompute
% it based on nConv zero-padding point
...
xlabel('Time (s)'), ylabel('Frequency (Hz)')
title('Time-frequency plot of a nonlinear (Laplacian) chirp')
