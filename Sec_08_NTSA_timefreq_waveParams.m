...
figure(1), clf
for i=1:length(num_cycles)
    subplot(4,1,i)
    s = num_cycles(i) / (2*pi*frex); % width of gaussian
...
set(gca,'clim',[-3 3],'ydir','normal','xlim',[-300 1000])
title('TF power data with variable number of wavelet cycles')
subtitle(['Log-spaced between ' num2str(range_cycles(1)) ' - ' num2str(range_cycles(2)) ' Hz' ])
xlabel('Time (ms)'), ylabel('Frequency (Hz)')
colormap jet
colorbar
