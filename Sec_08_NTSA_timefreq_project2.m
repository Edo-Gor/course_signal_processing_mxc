...
for fi=1:nFrex
    ...
    basepow = mean(mean(powdat(bidx(1):bidx(2),:),2),1);
    tf(fi,:) = 10*log10( mean(powdat,2)/basepow );
...
title('Filter gains by frequency band')
colormap jet
...
title('Time-frequency plot from electrode 30 via filter-Hilbert analysis')
colormap jet
colorbar
