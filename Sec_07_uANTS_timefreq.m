...
% parameters
srate = 1000;         % in hz
time  = -2:1/srate:2; % BEST PRACTICE is to have time=0 at the center of the wavelet also more practical to have a wavelet with odd number of timepoints
...
% confirm that the shape of the power spectrum of a Morlet wavelet is a
% Gaussian 

pnts = length(time);

mwX = 2*abs(fft( mw )/pnts);
...
legend({'kernel';'signal';'result'})

%%% QUESTION: What does the third input in conv() mean?
%             What happens if you remove it?
%             > there's a shift forward equivalent to half the length of
%             the kernel because with the default options the zero-padded
%             'wings' are not cut off, and also we are not soecifying any
%             time vector
...
kflip = kernel(end:-1:1);   %-mean(kernel); or % /lenth(kflip);
...
set(gca,'ylim',[-1 1]*3.5,'xlim',[0 length(dat4conv)+1])
plot([1 1]*(half_kern+1),get(gca,'ylim'),'k--')
plot([1 1]*(length(dat4conv)-2),get(gca,'ylim'),'k--')
legend({'Signal';'Kernel (flip)';'Convolution'})

% run convolution
for ti=half_kern+1:length(dat4conv)-half_kern
...
halfK = floor(nkern/2);     % half the kernel to cut the wings in the end
...
legend({'Kernel';'Signal';'Convolved MatLab';'Convolved manually'})
...
%             > it is relevant in the MatLab function because the order of
%             inputs must be signal-kernel, if you swap them you basically
%             trim the signal based on the kernel length, rather than the 
%             other way around; the order in the manual implementation is
...
%              > that the temporal precision of v1 ERPs oscillates between
%              5 and 10 ms, if you consider larger kernel windows, you're
%              going to average nearly everything out 
...
plot(hz,abs(dataX)./max(abs(dataX)),'linew',2); % normalized for visualization
plot(hz,abs(kernX),'linew',2);
plot(hz,abs(dataX.*kernX)./max(abs(dataX)),'linew',2)

set(gca,'xlim',[0 150])
xlabel('Frequency (Hz)'), ylabel('Amplitude (norm.)')
legend({'Original signal';['Kernel ',num2str(h),' FWHM'];'Convolution result'})
...
%             > nope, it just shows the positive frequencies; since we are
%             taking the FFT of a complex singal, the spectrum won't be
%             symmetric (of course here the negative side is empty because
%             the components of the csw are orthogonal, but not all the
%             complex signals have empty negative frequencies)
% 
...
% Step 2.5: normalize the wavelet (try it without this step!) to avoid
% boosting the energy artificially
...
plot(hz,abs(dataX(1:length(hz))),'b','linew',2)

% plot power spectrum of wavelet
plot(hz,abs(kernX(1:length(hz))).*max(abs(dataX))/2,'linew',2)

% plot power spectrum of convolution result
plot(hz,abs(convresX(1:length(hz))),'linew',2)
...
subplot(212), hold on
plot(timevec,data,'b','linew',2)
plot(timevec,real(convres),'linew',2)
set(gca,'xlim',[-.1 1.3])
...
subplot(313)
plot(timevec,angle(convres),'linew',2)
xlabel('Time (ms)'), ylabel('Phase (rad.)')
set(gca,'xlim',[-.1 1.4])
title('Signal phase angles')

% notice the phase-slip at the low power window, probably due to the
% numerical error for small amplitudes rather than to a physiological true
% event
...
% extract more data (i.e. from all trials)
data = double( squeeze( csd(6,:,:) ) );

% reshape the data to be 1D (concatenate trials in one super-trial)
dataR = reshape(data,1,[]);  % = reshape(data,1,prod(size(data)))
...
for fi=1:num_freq
    ...
    as = ifft( dataX.*cmwX );  % 'analytic signal' in calculus is a complex valued signal that is fully differentiable
...
title('Your first time-frequency plot!')
colormap jet
...
% compute ITPC and preferred phase angle (second measure is usually not used but here it is)
...
for fi=1:num_frex
    ...
    % question: is this next line necessary? No if you're interested only in phases
...
for i=1:4
    ...
    %%% time domain
    subplot(4,2,i*2-1)
    s = num_cycles(i)/(2*pi*frex) ; % Gaussian width as number-of-cycles (don't forget to normalize by 2pif!)
    plot(time, exp( (-time.^2) ./ (2*s^2) ),'b','linew',3)
    ...
    % plot it
    plot(hz,abs(cmwX(1:length(hz))),'r','linew',3)
    set(gca,'xlim',[0 20])
    xlabel('Frequency (Hz)')
...
for cyclei=1:length(num_cycles)
    ...
    for fi=1:length(frex)
        ...
        as = ifft(cmwX.*dataX);                 % ifft
        as = as(half_wave+1:end-half_wave);     % trim wings
        as = reshape(as,EEG.pnts,EEG.trials);   % reshape as trials
...
% which is quite common in the litarature (i.e., lower frequencies less sycles, and 
% higher frequencies more cycles, in a log relationship)

% set a few different wavelet widths (number of wavelet cycles)
range_cycles = [ 4 13 ];

% other wavelet parameters; notice that if you use just the logspace(x,y,z)
% function matlab interprets it as logspaced numbers from 10^x to 10^y in z
% steps (hence the embedding of x and y in the log10() function)
...
for fi=1:length(frex)
    ...
    waveletX = fft(wavelet,nConv);
    waveletX = waveletX ./ max(waveletX); % not necessary here because of baseline comparison
...
for basei=1:size(tf,1)
    ...
    % decibel
    tf(basei,:,:) = 10*log10( activity ./ repmat(baseline',[1 EEG.pnts]) ); % can also use bsxfun()
end
...
%             better filter kernel? First answer, then try it! Increased
...
figure(15), clf
subplot(211)
plot(rts,'ks-','markersize',10,'markerfacecolor','k')
...
figure(16), clf
subplot(211)
plot(frex,b,'rs-','markersize',10,'markerfacecolor','k')
xlabel('Frequency (Hz)')
...
%   > at 8 Hz there is a positive correlation between energy in the signal
%   and reaction times, while for 20 Hz there is a negative relationship
%   between energy in the signal and reaction times; i.e. more energy =
%   slower, less energy = faster, respectively
...
for fi=1:numfrex
    ...
    % extract power from all trials (not averaging)
    tf3d(fi,:,:) = abs(as).^2;
...
%             > for frequencies ~5 Hz there is a negative correlation with
%             RTs at around 300 ms and a positive correlation with RTs at
%             around 700 ms
% 
...
% these are the time points we want to save (instead of the full range from -.5 to 1.5 secs)
...
numfrex    = 42; % number of frequencies between lowest and highest
...
for fi=1:numfrex
    ...
    cmw = exp( (1i*2*pi*frex(fi)*wavtime) - (4*log(2)*wavtime.^2/.3^2) );
...
% soft-coded parameters
freqrange  = [10 100]; % extract only these frequencies (in Hz)
numfrex    = 60;        % number of frequencies between lowest and highest
...
%             > no because of contourf()

%% Manually change the y-axis scaling to log

set(gca,'YScale','log')
set(gca,'ytick',round(frex(1:5:numfrex),1))
...
clear % a clear MATLAB workspace is a clear mental workspace :D
...
for chani=1:EEG.nbchan
    ...
    thisChanERP = mean(EEG.data(chani,:,:),3);
...
EEG.NPL = bsxfun(@minus,EEG.data,mean(EEG.data,3));
...
plot(EEG.times,mean(EEG.data(chanidx,:,:),3),'b','linew',2)
plot(EEG.times,mean(EEG.NPL(chanidx,:,:),3),'r','linew',2)
...
%             > A feature of course ! But a good sanity check (mind the computer rounding errors)
...
%             > only the early component at 5-15 Hz seems phase locked
...
%             > there's quite an overlap, which is natural
% 
%%% QUESTION: Does it make sense to compute the phase-locked ITPC? Why?
%             > well, the phase locked part of the signal should
%             have a strong ITPC because it's a phase-locked signal
