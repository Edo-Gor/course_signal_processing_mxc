...
for fi=1:N
    ...
    reconSig = reconSig + fourierSine*fourierCoefs(fi);
...
plot(signal,'b*-')
plot(real(reconSig),'ro','linew',2)     % not multiplying by N because already normalised after FFT
plot(real(ifft(fourierCoefsF))*N,'ko')
xlabel('Time (a.u.)')
legend({'Original signal';'Manual IFFT';'IFFT'})
