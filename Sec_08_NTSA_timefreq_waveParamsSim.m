...
plot(stime,signal,'linew',3)
plot(stime,sigamp,'--','linew',2)
xlabel('Time (s)')
...
subplot(223), hold on
plot(stime,squeeze(tf(:,fidx,:)),'linew',1.5)
plot(stime,sigamp/2,'k--','linew',1.5)
xlabel('Time (s)')
...
subplot(224), hold on
plot(frex,squeeze(tf(:,:,tidx)),'linew',1.5)
plot(linspace(0,srate,pnts),2*abs(fft(signal)/pnts),'k--','linew',1.5)
