...
times2save = -.2:.03:1.2; % in seconds
...
plot(times2save,tfdwns(fidx,:),'ro','markerfacecolor','r','markersize',5)
set(gca,'xlim',timevec([1 end]))
legend({'full res.';'downsampled'})
xlabel('Time (s.)'), ylabel('Power (dB)')
title(['Example from frequency = ' num2str(freq2plot) ' Hz'])
...
ylabel('megabytes')
title('MatLab buffer usage comparison')
subtitle('Downsampling to 0.3 Hz')
