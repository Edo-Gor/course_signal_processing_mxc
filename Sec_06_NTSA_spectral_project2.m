...
dataXpre = (2*abs(fft(EEG.data(:,tidxPre,:),nfft,2)/pnts)).^2;

dataXpst = (2*abs(fft(EEG.data(:,tidxPst,:),nfft,2)/pnts)).^2;

% band-limited power, average over power, then over trial (or the other way around, here it doesn't really matter)
