...
data = repmat(signal,ntrials,1); % repmat() basically repeat 'signal' for a 'ntrials x 1' vector
...
figure(1), clf
subplot(511)
plot(time,signal,'k','linew',3)     % plot the pure signal
set(gca,'ylim',[-1 1])
title('Pure signal')

subplot(5,1,2:4)
imagesc(data)                       % plot the data split by trials
set(gca,'clim',[-1 1]*noiseamp*2)
ylabel('Trial')
title('All trials: signal + noise')

subplot(515)
plot(time,mean(data),'k','linew',3) % plot the data averaged over trials
xlabel('Time (s)')
set(gca,'ylim',[-1 1])
...
contourf(timevec,1:16,squeeze(mean(csd,3)),40,'linecolor','none') % select whole csd; 4th input add contours
