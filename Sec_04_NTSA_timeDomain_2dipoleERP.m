...
% fixed IF signal for dipole1 (frex range 2-7 Hz)
...
for triali=1:EEG.trials
    ...
    % generate dipole activity (white noise in all chans)
    dipact = .02 * randn(size(lf.Gain,3),EEG.pnts);
    
    % dipole 1 (chirp by gaussian)
    fwhm = randn/10 + .3;
    gaus = exp( -4*log(2)*(EEG.times-1).^2 / fwhm^2 );
    dipact(diploc1,:) = IFsignal .* gaus; % pure signal, to include noise:
    % dipact(diploc1,:) = dipact(diploc1,:) + IFsignal .* gaus;
    
    % dipole 2 (just gaussian)
    fwhm = randn/10 + .3;
    gaus = exp( -4*log(2)*(EEG.times-1).^2 / fwhm^2 );
    dipact(diploc2,:) = gaus; % pure signal, as above
