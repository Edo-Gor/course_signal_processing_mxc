%%
%   COURSE: Neural signal processing and analysis: Zero to hero
%  SESSION: Introduction
%  TEACHER: Mike X Cohen, sincxpress.com
%

%% become familiar with the sample data used in class
%

%% start with the EEG data
load sampleEEGdata

% explore a bit... what are the different fields? What is the size of the data?
% How many channels/time points/trials?
% What is the earliest and last time point? 
% Where is time = 0?
% What is the sampling rate?
whos
EEG

% have a look to the channel position
axis square, grid on
plot3([EEG.chanlocs.X],[EEG.chanlocs.Y],[EEG.chanlocs.Z],'ko','markerfacecolor','b')

%% plot ERPs and topographical maps

% compute the ERP of each channel 
% (remember that the ERP is the time-domain average across all trials at each time point)
% data is a cube containing channels, timepoints and trials, we need them
% all and we need to specify which dimention we want to compute the mean
% over (here the trials, so dimension 3)
erp = mean(EEG.data(:,:,:),3);


% pick one channel and plot ERP for that channel; you can see the channels
% names in {EEG.chanlocs.labels}
chan2plot = 'fcz';

figure(1), clf
plot(EEG.times,erp( strcmpi({EEG.chanlocs.labels},chan2plot) ,:),'linew',2)
xlabel('Time (ms)'), ylabel('Activity (\muV)')
set(gca,'xlim',[-400 1200])

%% plot topographical maps

% select the timepoints you want to visualise
time2plot = 300; % in ms

% convert time in ms to time in indices, for two reason, EEG.times is -1000
% and the sampling rate is not 1 ms
EEG.times(300) % naively extract the 300th value of the time vector
EEG.times(1)   % check the first element of that vector (-1000)
[~,tidx] = min(abs(EEG.times-time2plot));
EEG.times(tidx) % check the time in ms (300.78)

% note that [M,I] = min(X) returns the indices into operating dimension 
% corresponding to the minimum values. If X contains more than one 
% element with the minimum value, the index of the first one is returned.

% make a topographical map of the ERP at only this time point.
figure(2), clf
topoplotIndie(erp(:,tidx),EEG.chanlocs);
title([ 'ERP from ' num2str(time2plot) ' ms' ])
set(gca,'clim',[-8,8])
colormap jet;
colorbar

%%

%% now for sample CSD V1 data
clear,clc
load v1_laminar

% check out the variables in this mat file, using the function whos
% If you don't know what variables are in this file vs. already in the workspace,
%  you can clear the workspace and then load the file in again.

% the data are in variable csd. What is the size? How many channels/time points/trials?
whos

% csd contains a cube of data from 16 channels, 1527 timepoints, 200 trials

%%

% plot ERP from channel 7 in one line of code! note that channels aroud 6-7
% corresponds to the layer IV of V1
figure(3), clf
plot(timevec,mean(csd(7,:,:),3))
hold on
plot(get(gca,'xlim'),[0 0],'k--')
plot([0 0],get(gca,'ylim'),'k--')
plot([0 0]+.5,get(gca,'ylim'),'k--')
xlabel('Time (s)'), ylabel('Activity (\muV)')
set(gca,'xlim',[-.1 1.4])


% plot depth-by-time image of ERP by averaging over trials ('equivalent' of
% the topografical map)
figure(4), clf
contourf(timevec,1:16,squeeze(mean(csd,3)),40,'linecolor','none')
set(gca,'xlim',[0 1.3])
colormap jet;
xlabel('Time (sec.)'), ylabel('Cortical depth')

%% done.
